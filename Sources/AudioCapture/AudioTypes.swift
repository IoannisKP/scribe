...
///
/// Persisted so that regeneration can never overwrite a title the user chose.
public enum SessionTitleSource: String, Codable, Equatable, Sendable {
    /// The original date-and-time folder name. Always a safe outcome.
    case dateTime

    /// Derived from an already generated and already consented summary.
    case summary

    /// Generated by a provider running on this Mac, so nothing left it.
    case localModel

    /// Extracted from the transcript with no model at all.
    case keywords

    /// Generated by a cloud provider, only ever with explicit opt-in.
    case cloudModel

    /// Typed by the user. Never regenerated over.
    case userSet

    /// Whether an automatic pass is allowed to replace this title.
    public var isReplaceableByGeneration: Bool {
        switch self {
        case .dateTime, .keywords:
            true
        case .summary, .localModel, .cloudModel, .userSet:
            false
        }
    }
}
